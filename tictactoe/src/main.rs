use std::{cell, env::current_exe, io};
...
fn get_player_move(cuurent_player:char, board:&Board)-> (usize,usize){
    loop{
        println!("Player {} input (row,col)", cuurent_player);
...
fn play_game(){
    ...
    loop{
        ...
        let (row,cel) = get_player_move(current_player, &board);
        board[row][cel]=current_player;
